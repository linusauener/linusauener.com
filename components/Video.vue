<template>
    <div class="w-full h-full bg-gray-900 relative overflow-hidden flex flex-col items-center justify-center px-10">
        ...
        <video :id="`bg-video-${index}`" class="video-bg h-full w-full object-cover absolute z-10" muted playsinline :src="`/videos/${url}#t=0.001`"></video>
        <div class="top-0 w-full h-full object-cover absolute z-20  backdrop-filter backdrop-blur-lg backdrop-contrast-75"></div>
        <video :id="`fg-video-${index}`" @ended="stopped()" class="relative z-30 shadow-2xl rounded-lg" muted playsinline :src="`/videos/${url}#t=0.001`"></video>
    </div>
</template>
